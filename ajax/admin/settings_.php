<?php
    ...
    if($_POST["methodName"] == "settingsStart"){      // Загрузка списка позиций меню исходя их типа и прав пользователя
        ...
        if(TYPE == 1) $TEXT .= "<div class = 'list_item' id = 'settings_menu_products' onClick = 'settingsLoad(\"products\");'>Параметры товаров</div>";
        if(TYPE == 1) $TEXT .= "<div class = 'list_item' id = 'settings_menu_offs'     onClick = 'settingsLoad(\"offs\");'    >Основания списаний</div>";
        if(TYPE == 1) $TEXT .= "<div class = 'list_item' id = 'settings_menu_money'    onClick = 'settingsLoad(\"money\");'   >Разменные деньги</div>";
        if(TYPE == 1) $TEXT .= "<div class = 'list_item' id = 'settings_menu_delivery' onClick = 'settingsLoad(\"delivery\");'>Транспортные компании</div>";
        if(TYPE == 1) $TEXT .= "<div class = 'list_item' id = 'settings_menu_season'   onClick = 'settingsLoad(\"season\");'  >Сезонное хранение</div>";
        if(TYPE == 1) $TEXT .= "<div class = 'list_item' id = 'settings_menu_naklad'   onClick = 'settingsLoad(\"naklad\");'  >Товарная накладная</div>";
        if(TYPE == 1) $TEXT .= "<div class = 'list_item' id = 'settings_menu_file'     onClick = 'settingsLoad(\"file\");'    >Активный файл (ссылка)</div>";
        if(TYPE == 1) $TEXT .= "<div class = 'list_item' id = 'settings_menu_massa'    onClick = 'settingsLoad(\"massa\");'   >Массовая загрузка</div>";
    ...
    if($_POST["methodName"] == "settingsLoad"){      // Загрузка пункта меню настроек
        ...
        if($param == "providers"){       // Поставщики
            ...
            $sql = mysqli_query($CONNECTION, "SELECT * FROM provider WHERE status = 1");
    ...
    if($_POST["methodName"] == "settingsWorkerAdd"){      // Добавление нового пользователя
        ...
        if($data["id"] > 0) echo -1;
            ...
            mysqli_query($CONNECTION, "INSERT INTO user_root
                ...
                ('$id', '11111111111111111', '11111111111111', '111111111', '11111', '11111111111', '11111111111111111', '11111111')");
            $TEXT = "
